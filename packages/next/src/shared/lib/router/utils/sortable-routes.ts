 ...
 */
function compareRouteSegments(pathA: string, pathB: string): number {
...
export function sortPageObjects<T>(
  ...
  // In-place sort the pages.
  pages.sort(compareRouteSegments)

  // Map the sorted pages back to the original objects.
  return pages.map((page) => objects[indexes[page]])
}
